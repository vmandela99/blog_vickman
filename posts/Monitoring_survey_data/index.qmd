...
\`\`\`{# Install and load necessary packages} \# install.packages(c("dplyr", "lubridate", "testthat")) library(dplyr) library(lubridate) library(testthat)
...
Now we generate the sample data and schedule a test at a certain time daily. For this case we check every 2:00am.

\`\`\`{# Generate a sample dataset} generate_sample_dataset()

# Schedule the script to run every day at a specific time (e.g., 2:00 AM)

while (TRUE) { current_time \<- as.numeric(format(Sys.time(), "%H%M"))

\# Check if it's time to run the script (e.g., 2:00 AM) if (current_time \>= 200 && current_time \< 201) { test_file("data_quality_checks.R") \# Run the tests check_data_quality() Sys.sleep(86400) \# Sleep for 24 hours (86400 seconds) before checking again } else { Sys.sleep(60) \# Sleep for 1 minute before checking again } }
